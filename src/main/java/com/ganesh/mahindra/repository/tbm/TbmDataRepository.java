...
public interface TbmDataRepository extends JpaRepository<TbmData, Integer>{
	...
	@Query(value="select m.machine_id,m.machine_name,m.machine_no,coalesce((l.location_id),0)as location_id,coalesce((l.location),0)as location_name,coalesce((i.item_id),0)as item_id ,coalesce((i.item_name),0)as item_name,coalesce((i.frequency),0)as frequency,coalesce((select t.t_tbm_id from t_tbm_machine t where i.item_id=t.item_id ORDER BY t.t_tbm_id DESC LIMIT 1),0) as t_tbm_id," + 
